...
| **`armada guild verify`** | Reserved by name and refused by name. **`edit` is no longer reserved**: it was built to the contract it was claimed under — open a guild file, validate it, commit it — alongside `guild browse` and `guild delete` (`PLAN.md` §15.3.4). Its validation reads each file the way the thing that consumes it does; `verify` is the wider cross-check, and still unbuilt. |
