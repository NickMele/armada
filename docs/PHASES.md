...
| **`clean --orphaned --force-rebuild` was a stub** returning `bad_invocation`. | `PLAN.md` §4.3 specifies it as the way out of a `char.db` char cannot read, and *the recovery path must not need the thing that is broken* — so it runs at the entrypoint, before `App` opens anything. It moves the unreadable file aside rather than deleting it, and carries the old namespace across when that is still legible, so resources already stamped stay reapable. The shipped invocation is `char clean --all --orphaned --force-rebuild`: the pass enumerates every labelled resource on the daemon and removes across namespaces, which is machine-scoped work and so has to be asked for rather than inferred from a workspace-scoped command line. `--dry-run` previews it, replacement database included. |
