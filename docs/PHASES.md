...
>
> Armada is one system with four modules: **Manifest**, **Guild**, **Fleet**, **Surface**.
...
| **M3** | Fleet + Surface | Sessions, budgets, inbox — **and the orchestrator on top** | the product |
...
**M3 ships Fleet and Surface together, as one milestone.** An earlier draft of this plan had
...
| **Crates** | `core`, `manifest`, `guild`, `fleet`, `surface` — mirroring [`PLAN.md`](PLAN.md)'s module structure. Today's `charkit-core` + `charkit-adapters` become `manifest`; today's `charkit-cli` becomes `surface`. |
...
### 8.5 M3 — Fleet and the orchestrator

The product. Everything before this exists to make this possible.

**Fleet — the agents you do not talk to.** A session is a UUID, a git worktree, a port block
and a transcript; the process is temporary and the session is not (§9.1 F1). Fleet mints the
UUID before anything runs, so ownership is recorded up front and cleanup can find the session
afterwards even if the directory is gone.

| Verb | Contract |
|---|---|
| `spawn` | Classify → worktree → `manifest init` → budgeted headless turn. Classification is Fleet's, not the orchestrator's: it is needed the moment a session can be spawned. |
...
| `answer` | Resume a worker with your decision. |
| `open` | Prints the worktree path and resume command. Armada does not own a terminal — cmux or the Claude app opens it. |
| `kill` | `manifest clean` → drop the worktree → release the port block, in that order. |

**Surface — the one agent you do talk to.** Typing `armada` launches a Claude Code session
running an orchestrator persona from your guild, with Armada's MCP server as its toolbelt. It
needs no interface work at all, which is why it lands here rather than after everything else.
Because the orchestrator *is* a session, a TUI or an app that hosts it later is a rendering
choice and not an architecture change.
...
nightly job is flaky", and two isolated sessions run, report, and bring you the one decision
...
writes only a thin index of session metadata on top. This also retired two earlier designs —
...
exactly — the orchestrator is interactive and the workers are headless — but it means monitors
can never be a worker-side mechanism.
...
|---|---|---|
| **Manifest stalls** | Fleet and Surface are more interesting. Manifest stops at three verbs and M4 never unblocks, because `check` never lands. | Manifest's remaining verbs are milestones, not background work. M4's blocker is named in §8.6 for this reason. |
...
| **Orchestrator context bloat** | If the orchestrator reads worker transcripts it fills its window in three sessions and starts forgetting the fleet. | Structural: the orchestrator reads **summaries only**; probe is a separate cheap model. A design constraint, not a tuning knob. |
...
2. **Nothing points upward.** Manifest may not reference Fleet; Guild may not reference Surface.
...
3. **Manifest stays agent-agnostic.** It is the bottom of the stack precisely because it knows
   nothing about agents. A convenience that leaks a session id into Manifest is not a
   convenience.
