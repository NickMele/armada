...
| [024](024-the-relay-does-not-fire.md) | The relay does not fire under `--print` | BUG | fleet | `020` §1 decided the Drone's `Stop` hook ticks the Job; measured, it never runs — `SessionStart` fires and `Stop` never appears, so every Job sits `SILENT` until somebody types `fleet tick` |
