...
| [020](020-the-tui-decided.md) | The TUI, decided | RESERVED | helm | Nine decisions from a design session after real use; **eight of nine are built** — the Stop hook drives tick, an action with a duration says `ABORTING · docker 12s…` on every surface that draws a Job, bare `armada` is a front door listing the five modules, and `↵` opens a Job in a cmux workspace with the panes still up. **The one left is the signals listing**, which waits on [021](021-the-work-hierarchy.md)'s rename rather than on effort |
...
| [034](034-the-job-daemon-lands-the-work.md) | The Job daemon lands the work | RESERVED | fleet | A daemon pushes each finished Job's branch, opens a PR, merges it when CI is green, re-runs the checks on `main`, reaps the workspace and tells every running Job that `main` moved — because the last step of every workflow ended in a person, and the orchestrator kept committing on Jobs' behalf |
