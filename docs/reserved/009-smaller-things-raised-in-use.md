...
| 2 | `config scan` offers the agent hand-over or nothing | Three options: build it with an agent, **write a blank `armada.yml` and open it in `$EDITOR`**, or stop. The middle one is missing and is what a reader who knows their own repository wants. **Half answered by [007](007-scanner-should-propose.md)**: the middle option exists and writes the *provable* config rather than a blank one, which is strictly more useful. What is still missing is opening it in `$EDITOR` afterwards |
