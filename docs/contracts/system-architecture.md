...
| `--permission-mode default` with Armada's permission tool, plus an explicit allowlist | A headless Drone hanging on a prompt, and a call outside the allowlist nobody decided | Spawn config. The tool answers every such call; one that fails leaves the call unrun |
