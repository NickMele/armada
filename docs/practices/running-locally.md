...
caught up, it merges `main` in and reruns the Checks the combination hits before
merging. The design is [Merge line](../capabilities/merge-line.md).

**When you run it:** from the branch's own worktree, after your Checks passed and
the owner said merge. Never `gh pr merge` or a push to `main` by hand, which
lands a combination nothing checked.
...
Until a build carrying it is installed, point `ARMADA_LAND_ARMADA` at
`target/debug/armada`. Reruns build in a fresh worktree seeded from the main
...
| 5 | Conflict | Merge `origin/main` in, commit, push, preflight and land; it keeps its place |
...
`python3 scripts/test_land.py` proves it against a throwaway repository with a
stub `gh` and `armada`, and needs nothing built or signed in.
