...
- **[shadcn-and-tokens]** How shadcn primitives take their values from the
  ...
  the two compose.
- **[eslint-for-one-rule]** Is `@typescript-eslint/no-floating-promises` worth
  a linter this repository otherwise has no use for? Every project sets
  `strict: true` and the four unused-code checks and `noUncheckedIndexedAccess`
  besides, so the compiler now says everything it can say here. It cannot say
  this one: a dropped promise is well-typed, and **`void promise()` is the
  discipline that stands in for the check** — seventy call sites outside tests
  and stories, most of them the one-line commands in
  `apps/desktop/src/renderer/src/commands.ts`. Nothing enforces it, so the
  seventy-first is a silently unhandled rejection.

  What decides it is not the rule, which is a good rule. It is that adopting it
  means adopting ESLint, and a generic config would restate `strict` and then
  argue with a house style `cargo xtask verify-foundations` already enforces —
  design tokens over literals, story titles matching paths, generated
  vocabulary agreeing with the registries. So the shape has to be a config that
  turns nearly everything off, which is a toolchain to keep working for one
  rule. `[clippy-as-a-check]` in `rust.md` is the same trade on the other side
  of the repository and neither is settled; whatever answers one is likely to
  answer both. The narrower alternative is a `verify-foundations` rule that
  reads for a promise-returning call with no `void`, `await` or `.catch` —
  cheaper to own, and it would not know a type, so it would have to work by
  spelling rather than by resolution.
