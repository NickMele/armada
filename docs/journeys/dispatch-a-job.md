...
- **A Convoy can be a peer node in dependencies.** It carries and is the target of `depends_on` / `blocks` like any other Job; only decomposition is excluded, and both the Convoy and Job pages were reworded from "uses none of" to "does not decompose." The links sequence peers, and peers are not children, so having no children never ruled out sequencing a Convoy against a peer. The Job proposer may also emit a mixed decomposition, with a Convoy as one node of the graph. Atomicity is not part of what it proposes: a Job's `atomic` flag is settled by the workflow's declaring step, and [Convoy](../concepts/convoy.md) owns how a Convoy is arrived at.
