...
**Steps along a spine, one Plan node hanging off it, and a loop returning above.** The steps are the ones the Job's own workflow file declares, read off the frozen workflow rather than off a catalogue, so a bug Job draws three and a revert Job two. Under the step that recorded the plan hangs a single node for the plan, carrying the group count and the task count and nothing else.

**Pressing the Plan node opens the Plan destination, where the plan is drawn whole.** Why: the owner, 25 September 2026. Until then this canvas drew the entire plan — a node per group under the step that wrote it, a node per task beside its group. **The cost is the second edge**, which said *and this step worked it*: it was his own decision two days earlier, and he reversed it knowingly. There are no step nodes on Plan, so nothing there can draw that relation, and there is nothing left here for it to arrive at. The screen being readable beat the picture being complete.
...
### Plan — the split, as a graph or as a list

**Two views of one plan, Graph and List, and a toggle between them.** Why: the owner, 25 September 2026, reading the split as a list and asking for views of its own. Graph is the group and task graph that came off Workflow, with the step row left behind — a column of groups, each holding its own tasks, and nothing joining one group to another. List is the plan board: the card is the group, the boundary is under it, and what a Drone will be told is one press away.
