...
- **Editing during a running Job is safe — no warning, no lock.** Settled 2026-08-21: a Job resolves its config from a snapshot taken at spawn, so it is judged against the Checks it started with and an edit on this surface cannot reach it. Same mechanism that stops a Drone granting itself a Command. The next Job picks up the new file; the running one does not.
...
- **[runedit-checks-snapshot-confirm]** Does the Job-config snapshot at spawn already cover Checks, or only Commands?
  Editing during a running Job is settled as safe because a Job resolves its config from a snapshot taken at spawn. **One thing to confirm:** the snapshot is stated for Commands — it needs to cover Checks too, or a Job frozen at a gate could be judged against a Check that changed under it.
