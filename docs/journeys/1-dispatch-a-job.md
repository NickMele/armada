...
- **[job-board-graph-view]** Does the Job Board need a graph view for DAG dependencies?
  Job dependencies are a full DAG — Jobs branch and fan in, and Fleet schedules topologically. The brief states directly that the Job Board needs a graph view, not just a flat list, but nothing specifies what that view is: a separate surface, a toggle on the existing list, or dependency affordances on the rows themselves. The Job Board's own layout section describes a list only. This bears on two further unresolved surface questions from the same review: whether a Convoy in the graph view renders as an isolated node, is omitted, or expands its declared workspaces (a Convoy has no `dependencies` by construction, though it may now be a peer in the DAG); and how a Board row visually distinguishes a Convoy from a single-workspace Job at all, since origin tagging has no value expressing a Convoy's shape and never will — shape is derived from `write_targets` and `atomic` rather than stored.
