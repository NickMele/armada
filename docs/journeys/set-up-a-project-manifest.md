...
**The rail's picker names a set-up repository by its Manifest id alone.** Fleet refuses to serve one id twice, so nothing else is needed to tell two apart. A repository not yet set up has no id and reads as its folder, with its parent only where two such folders share a name. Every Bridge follows an add made anywhere, and the Board lists what the picker names — every repository's Jobs on All, or the picked one's — per [Job Board](../concepts/job-board.md).
