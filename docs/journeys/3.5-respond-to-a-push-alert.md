...
- **The header states that the Drone is paused rather than offering to stop it.** Fleet never auto-kills, so by the time this screen exists the state is already held as-is.
...
| **Noted** | Nothing is waiting on you | Activity Feed only. Never interrupts, never queues |
...
Paused, always — Fleet never auto-kills. Worktree and state held as-is. Killing is exclusively a human action (rung 2, or directly from Alerts), and always prompts a cleanup step rather than happening silently.
