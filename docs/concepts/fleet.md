...
**A comment on a pull request is untrusted input.** It is written by whoever can see the pull request, and it carries no further than the value Fleet read it into: how many there are reaches the Job's log and what they say does not. Nothing yet puts one in front of a Drone.
