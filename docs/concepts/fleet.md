...
**A gate asks the same memory and disk before each Check after the first.** How many of a step's Checks run at once is a fourth limit, changed from Bridge the same way; a short machine makes the next Check wait for a running one to finish. The first always starts, so a short machine slows a gate and never stops one.
