...
**Adoption is of the process and the record, never of the session.** Both of a Drone's pipes died with the Fleet that held them and nothing can reopen another process's stdin or stdout. So an adopted Drone can be ended, and can submit — evidence travels over the loopback rather than the pipe, and nothing a Drone says gates its own step, so it can finish the step it was on and be gated normally. It cannot be poked, redirected, told a verdict or handed a step back, and nothing it says from here is recorded. **So the acts a person is offered on its Job are the ones that need no pipe**: a redirect and a gate re-run are withheld, and what answers a stopped step is the restart, which ends the unreadable Drone and puts a fresh one on the step. Fleet reads the condition — nothing is reading this Drone — rather than the fact of adoption, so a second way to lose a pipe withholds the same two acts. See [Job](job.md).

**Two costs are paid rather than avoided.** The turns between the last line Fleet read and the restart are unrecoverable, and so is what they spent — the harness reports a run's cost on its terminating line, so an adopted Drone's recorded spend is an undercount. And Fleet is deaf rather than the Drone being silent, so the liveness ladder eventually escalates a healthy adopted Drone; the instinct is right and the trigger was wrong — `stalled` is a Drone that stopped producing, and this one is producing into a pipe with no reader — so the badge it escalates under is `unheard`.
...
**Asking for a path is its opposite on both counts.** A [Judge](judge.md) answers it, and a model call has a budget a person's wait does not — so the call is held open and the answer is in the reply rather than in a later turn. The Checks tool is the other way round: a build outlasts what a client waits on one call, so it answers at once and reports as a later turn. What it may not do is narrow: the tool has no field for a path to hand back.
...
**The Drone is told which Checks gate its step, by name. It is not told what any of them runs.**
...
So what remains is narrower. No block a Drone is given is written from a Check's command, and no prompt names a bar. What the Drone has instead is a third MCP tool: it asks, Fleet runs the step's own frozen Checks in the Drone's worktree, and the Drone is told what each one did and where its output was written.

**The offer names the Checks the call would run, by name and never by command.** Naming them is the difference between a Drone that can decide whether to spend a call and one that can only spend one to find out — which is the guessing the tool exists to replace, arriving a step earlier. The names are the same words the report comes back carrying, so the two line up row for row. **It is a list and not a menu**: one ask runs the step's whole declaration, and a Drone offered a choice of *which* Checks could skip the one that would have caught its mistake. The tool takes exactly one argument and it is not that choice — `only_what_changed` says how much of the tree each Check reads, never which of them run. Fleet derives the paths from the worktree diff, so the Drone narrows nothing itself; a Drone supplying paths would be choosing its own scope, which is the same move one level down. A narrowed answer says the parts that changed hold, not that the repository does. A step declaring no Checks is offered no block at all rather than an empty list.

**A named list is a promise, so where it may not be kept the block says so.** A Check the Manifest scopes to certain paths comes back skipped rather than run when the work has not touched them, and a Drone that read the name, called for exactly that Check and got a skip is back where it started — deciding after the call what the offer exists to let it decide before. The caveat is rendered only on a step that holds such a Check, and it is written about files rather than about the schema key that scopes them. **The answer is a signal and never a pass** — no step advances on it, nothing is recorded against the step, and the gate runs the Checks again for itself. Fleet suspends the convergence clocks while it runs them, because a Drone waiting on Fleet is not silent and is not thrashing; what bounds the cost instead is a count per step and a refusal while one is already running.
...
- **[drone-builtin-tools-confinement]** How is a Drone's toolset actually confined, given that `--allowedTools` removes none of the built-in tools? `--strict-mcp-config` bounds MCP servers only, not the thirty built-in tools the CLI ships with. **The first live instance arrived 8 Sept 2026, and it was not a security one.** A Drone on `01M21BKVPW002DC0ATD1X9T0VF` reached the built-in `Agent` tool, delegated its exploration to a subagent, and ended its turn to wait for the report — which reaches a session as a notification a headless Drone between turns cannot be handed. It died waiting, 21 seconds in, having submitted nothing. Fleet gives that turn back now rather than reaping it, so the capability is accommodated rather than removed, and this question is unchanged: nothing bounds which built-ins a Drone may reach, and the next instance may not be one that can be accommodated.
