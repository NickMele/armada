...
Two consequences follow, and both are paid rather than avoided. **Approving does not resume where the Job was**: the slot the gate gave up is very often another Job's by the time a person answers, so the approved Job goes back in the queue and a Drone is put on its next step when a slot is free. And **sending the step back across the gate does not work yet** — a person doing that is briefing the next Drone rather than talking to the one that did the work, and until the boundary can carry a note there is nowhere for it to wait, so the act refuses. That is a known interval, not a broken gate; approve or reject in the meantime.
