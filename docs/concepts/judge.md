...
- **Gaming check** names its baseline with `baseline_ref`. `flag_if` names the patterns that make work look green while gutting it — scope narrowed, assertions weakened, tests skipped, tests deleted, and a change to the configuration a Check's command resolves through. The last was added because editing `package.json` `scripts.test` honours a frozen `run:` string exactly while narrowing the gate. [Workflow](workflow.md) carries the set.
