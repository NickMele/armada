...
Navigation carries Overview, Job Board, Alerts, Doctor, Manifest, Cleanup, Settings and Studios. Helm is not one of them — it is a dock beside the content on every surface, toggled by `⌘J` rather than a rail digit, and above the layout breakpoint a closed dock draws nothing at all: the title row's own Helm button is the one way back. See [Helm](helm.md).
