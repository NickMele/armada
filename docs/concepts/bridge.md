...
The rail carries Overview, Job Board, Alerts, Doctor, Manifest and Held worktrees. Helm sits below them.

**Held worktrees was last because it was newest, and that is the rule rather than a placement.** A surface joins at the end, so `⌘1`–`⌘4` kept reaching what they reached when Held worktrees arrived, and only the sibling beneath the rule took a new digit — Helm went to `⌘6`. **Overview is the one surface that broke that rule**: it is where Bridge opens (#921), so it joined first rather than last, taking `⌘1` and pushing every other surface's digit down by one — Job Board `⌘2`, Alerts `⌘3`, Doctor `⌘4`, Manifest `⌘5`, Held worktrees `⌘6`. See `../contracts/design-system.md`, Two tiers.
