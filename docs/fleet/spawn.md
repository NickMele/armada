...
Start an isolated agent session on a task.
...
| `--name <name>` | string | derived from task | The session's handle. Must be unique among live sessions. |
...
1. **Classify.** One cheap model call turns the task text into a workflow name, with the
   ...
   orchestrator, because it is needed the moment a session can be spawned
...
6. **Register the session** in `~/.armada/sessions/` — uuid, name, worktree, branch, port block,
   budget.
...
`--json` returns the session record: `uuid`, `name`, `workflow`, `confidence`, `worktree`,
...
| `claude` | Runs the session. |

## Exit codes

`0` spawned · `1` `tool_failed` — the worktree or `manifest init` failed · `2` `bad_invocation` — unknown workflow, or the session name is taken · `3` `bad_config` — no `armada.yml` · `6` `environment` — not a git repository, or the port pool is exhausted.
...
[`ls.md`](ls.md) · [`kill.md`](kill.md) · [`../surface/orchestrator.md`](../surface/orchestrator.md)
