...
End a session and release everything it owns.
...
```sh
armada fleet kill <session> [--keep-branch] [--keep-worktree] [--all-finished] [--json]
```
...
|---|---|---|---|
| `<session>` | session name | — | Which session. Required unless `--all-finished`. |
...
| `--all-finished` | flag | off | Kill every session whose workflow has terminated. |
...
3. **Mark the session ended** in the index.
...
The session index, and whatever [`../manifest/clean.md`](../manifest/clean.md) needs.

## Exit codes

`0` killed · `1` `tool_failed` — something would not release; **the session is still marked ended**, and `armada manifest clean --all` reclaims the remainder · `2` `bad_invocation` — unknown session.
