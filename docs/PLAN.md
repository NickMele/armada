...
### 15.3.1 … 15.3.9 — moved to `docs/reserved/`

**The reserved designs live in their own files now**, one per item, indexed in
[`docs/reserved/README.md`](reserved/README.md). They were moved because they have a different
lifecycle from this document: the spec is stable and read start to finish, while a reserved
design is written once, read on its own, and **added to every time somebody uses Armada and
finds something**. Nine of them had accreted under §15.3 — several belonging to Guild or
Manifest rather than to Helm — because that is where the first one landed, not because they
belonged there.

Each file carries an id, a title, a status and the module it belongs to, deliberately
machine-readable: when the task system in
