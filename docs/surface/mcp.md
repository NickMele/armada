...
| `fleet.probe` | — | Summarises one session's transcript with a cheap model. **Read-only; never resumes the worker.** |
| `fleet.answer` | [`../fleet/answer.md`](../fleet/answer.md) | Resumes a waiting session. |
...
**Worker sessions get a smaller set** — they may report and ask, not spawn. A worker able to
spawn workers is a fork bomb with a budget.

| Worker tool | Effect |
|---|---|
| `fleet.report` | Append progress to its own session record. |
...
[`orchestrator.md`](orchestrator.md) · [`inbox.md`](inbox.md)
