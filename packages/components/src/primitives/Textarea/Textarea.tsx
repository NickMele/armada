 ...
 * `textarea` is sanctioned by hard rule two since a Job's brief is
 * prose a person writes at length, and a single-line input fights its
 * content. The contract specifies no `Textarea` of its own, so surface,
 * edge, radius, type, focus, invalid and disabled all come from
 * `### Input` — a control behaving differently from an input for no
 * reason is two controls to learn.
 */
 ...
 *   three. The mockup draws an 88px well with no token for that number;
 *   a row count is the same well spelled in the type scale.
 * - It does not resize: a drag handle is chrome, and this is an
 *   instrument panel. A caller needing a taller well passes `rows`.
