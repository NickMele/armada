...
export const ESCALATION_REASON: Readonly<Record<string, Rendering | undefined>> = {
  ...
  "drone_gone": { verb: "the Drone was already gone", icon: null, badgeStatus: "escalated", statusToken: "--status-escalated", hint: null },
...
export const EVIDENCE_TYPE: Readonly<Record<string, Rendering | undefined>> = {
  ...
  "plan": { verb: "a plan", icon: null, badgeStatus: null, statusToken: null, hint: null },
  "review": { verb: "a review", icon: null, badgeStatus: null, statusToken: null, hint: null },
};
 ...
 */
export const GAMING_PATTERN_MEANING: Readonly<Record<string, { readonly headline: string; readonly explanation: string; readonly presets: readonly string[] } | undefined>> = {
  "assertion_weakened": { headline: "A test may have been weakened to make this step pass", explanation: "The gaming check thinks the Drone changed an existing test so it checks less than before, and nothing else in this change checks what was dropped. Code that should fail can then pass, so a person has to look at the lines below.", presets: ["It's not a test","Checked elsewhere in this change","The test checks the same thing"] },
  "test_scope_narrowed": { headline: "The tests may cover less than they did before this step", explanation: "The gaming check thinks the Drone changed the tests so they exercise less of the code than they did at the baseline step. Behaviour nothing tests any more can break without a Check noticing, so a person has to look at the lines below.", presets: ["The code they covered is gone","Covered by other tests in this change","The plan called for narrowing them"] },
  "tautological_test": { headline: "A test may not be able to fail", explanation: "The gaming check thinks this change leaves a test that cannot fail, because what it checks does not depend on the code under it. A green run of that test proves nothing, so a person has to look at the lines below.", presets: ["The test can fail","It's not a test","The code under it is checked elsewhere"] },
  "test_skipped": { headline: "A test that used to run is now skipped", explanation: "This change adds a skip marker to a test that ran before it. A skipped test reads as passing without running, so a person has to decide whether it should be skipped.", presets: ["The plan called for skipping it","It fails for a reason outside this change","The test did not run before this change"] },
  "test_deleted": { headline: "A test file was deleted in this step", explanation: "This change removes a whole test file, so nothing runs the tests it held. A person has to decide whether it should have gone.", presets: ["The code it tested was deleted too","Its tests moved to another file","The plan called for deleting it"] },
  "check_config_edited": { headline: "This step changed how a Check runs", explanation: "This change edits a file a Check's command runs through, such as a test script or its configuration. A Check can then pass while running less than it did, so a person has to look at the lines below.", presets: ["The Check runs as much as before","The plan called for this edit","The file is not one a Check runs through"] },
  "no_findings_on_substantial_diff": { headline: "The review found nothing in a big change", explanation: "The gaming check thinks a change this big should have had something to raise, so the review may not have read it. A person has to read the change before trusting the review.", presets: ["Most of it is moved or generated code","There was nothing to raise","The review shows it read the change"] },
  "findings_not_tied_to_changed_lines": { headline: "The review's findings may be about other code", explanation: "The gaming check thinks the findings name nothing this change touched, so the review may not have read it. A person has to check the findings against the change.", presets: ["They name lines this change touched","They're about code this change depends on","The review covered more than this change"] },
  "findings_generic": { headline: "A review's findings may fit any change", explanation: "The gaming check thinks the findings would read the same written about a different change, so they may not come from reading this one. A person has to check the findings against the change.", presets: ["The findings are specific to this change","The findings name real lines","A small change has little to say"] },
};
...
export const GAPS: readonly Gap[] = [
  ...
  { vocabulary: "escalation_reason", variant: "drone_gone", missing: ["icon"] },
  ...
  { vocabulary: "evidence_type", variant: "plan", missing: ["icon", "token"] },
  { vocabulary: "evidence_type", variant: "review", missing: ["icon", "token"] },
