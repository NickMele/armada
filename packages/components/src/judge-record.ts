 ...
 *
 * `docs/contracts/agent-copy.md` specifies a refusal as a fixed set of named
 * fields rather than prose, and says why: *"Generated text is specified by what
 * it must contain, never by what shape it takes. A structural rule gets
 * satisfied identically forever and produces twenty interchangeable
 * paragraphs."* The labels are Fleet's fixed copy, where uniformity is the
 * point.
 *
 * **Here rather than in the component that first drew them.** Two surfaces
 * render this record — `CriterionVerdicts`, which draws one Judge's answer
 * beneath the step it judged, and `JudgeRefusal`, which draws a panel's. Each
 * had its own copy of the three names, and a record whose field list lives in
 * two places is a record that can be renamed in one of them.
 *
 * **The markup is not shared and does not need to be.** The contract governs
 * which fields exist, what each holds and what they are called; how a rail
 * draws them against how a sheet does is a layout question with two honest
 * answers. What may not differ is the vocabulary.
 */
