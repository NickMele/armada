// The activity log — chapter two of a step's story, streaming, with nothing
// in front of it.
//
// **It was behind a tab** — the stream only drew after pressing *The
// drone's turns*, inside a four-tab region the drawing lacks. The chapter
// saying what is happening now was the one thing a person had to find;
// it is on the page now, filling as rows arrive.
// **Every line opens in place** — a row and its payload are one thing in
// the order it happened, so a payload opens beneath its row rather than
// replacing the list or opening a pane beside it. Which row is open is
// held here, a property of reading this log, not the Job.
// **The two names below are this app's own** — `detail-keys.ts` reached
// for `button.armada-entry`, a class `LogEntry` ships, so a rename in
// `packages/components` broke `h`/`l` silently. It now reads the region
// attribute and payload id this file gives every row.
