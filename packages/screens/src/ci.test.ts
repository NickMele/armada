...
describe("saidOf", () => {
  ...
  it("says when every check passed, and when nothing ran", () => {
    expect(saidOf({ kind: "all_passed", checks: 3, finished: 3 })).toBe("3 of 3 passed");
    expect(saidOf({ kind: "nothing_ran", checks: 0 })).toBe("Nothing ran");
  });

  it("says Fleet has not read it where there is no reading", () => {
    expect(saidOf(undefined)).toBe("Not read yet");
  });
});
...
describe("ciOf", () => {
  it("puts a conflict with main ahead of a failed run", () => {
    ...
    expect(ci.said).toBe("Conflicts with main");
