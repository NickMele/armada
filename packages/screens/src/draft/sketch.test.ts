...
function line(id: string): Drawing["strokes"][number] {
  return {
    id,
    points: [
      { x: 0, y: 0 },
...
describe("what is on the pad", () => {
  ...
  it("counts a line drawn by hand on its own, so a pad with no box still attaches", () => {
    expect(isDrawn(withStroke(NOTHING_DRAWN, line("s1")))).toBe(true);
  });
...
describe("drawing by hand", () => {
  it("puts each line on top of the last, in the order they were drawn", () => {
    const next = withStroke(withStroke(NOTHING_DRAWN, line("s1")), line("s2"));

    expect(next.strokes.map((one) => one.id)).toEqual(["s1", "s2"]);
  });

  it("keeps no line of fewer than two points, which is a press that went nowhere", () => {
    expect(withStroke(NOTHING_DRAWN, { id: "s1", points: [] })).toEqual(NOTHING_DRAWN);
    expect(withStroke(NOTHING_DRAWN, { id: "s1", points: [{ x: 4, y: 4 }] })).toEqual(NOTHING_DRAWN);
  });

  it("takes back the last line and no other", () => {
    const two = withStroke(withStroke(NOTHING_DRAWN, line("s1")), line("s2"));
  ...
  it("leaves the boxes alone, and a box taken off leaves the lines", () => {
    const drawn = withStroke(pad(), line("s1"));

    expect(drawn.shapes).toEqual(pad().shapes);
    expect(withoutShapes(drawn, ["b1", "b2"]).strokes.map((one) => one.id)).toEqual(["s1"]);
  });

  it("takes the first id free, so drawing and undoing does not count up forever", () => {
    expect(nextStrokeId(NOTHING_DRAWN)).toBe("s1");
    expect(nextStrokeId(withStroke(NOTHING_DRAWN, line("s1")))).toBe("s2");
    expect(nextStrokeId(withoutLastStroke(withStroke(NOTHING_DRAWN, line("s1"))))).toBe("s1");
  });
});
