...
pub fn tokens_json(tokens: &[Token]) -> String {
    // JSON cannot carry a comment, and the write hook reads the head of a file
    // to decide whether it is generated. So the marker is a field, first.
    let mut out = String::from(
        "{\n  \"_generated\": \"GENERATED by `cargo xtask verify-tokens --write`. \
         Do not hand-edit.\",\n  \"tokens\": [\n",
    );
