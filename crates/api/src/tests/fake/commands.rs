...
impl Commands for FakeDaemon {
    ...
    ///
    /// **It stops at `queued`, as `approve_dispatch` does.** The Drone on the
    /// next step is a turn's since `#456`, and a fake that answered `running`
    /// would be the one place a caller could read the old shape.
