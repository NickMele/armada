...
async fn the_tool_list_carries_every_tool_and_only_one_that_reports() {
    ...
    // question, a note and a drafted fix is a report. `permission` is not even the Drone's: the harness calls it,
    ...
    assert_eq!(answered.body.matches("\"inputSchema\"").count(), 13);
    assert_eq!(
        answered
            .body
