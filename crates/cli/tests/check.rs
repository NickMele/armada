...
use charkit_core::run::RunRecord;
use charkit_core::schedule::replay;
use std::path::Path;
use support::Machine;
...
fn the_dispatch_record_carries_the_argv_that_ran_and_no_environment_value() {
    ...
    let dispatch = record
        .journal
        .dispatches
        .get(&charkit_core::schedule::CheckId::new("app:fail"))
...
fn two_runs_of_one_failure_produce_the_same_signature() {
    ...
    for _ in 0..2 {
        ...
        let record = record(&repo, &run_id);
        digests.push(
            record.journal.dispatches[&charkit_core::schedule::CheckId::new("app:fail")]
                .signature
                .as_ref()
                .unwrap()
                .digest
