...
use charkit_adapters::db::Db;
use charkit_adapters::machine;
use charkit_adapters::process::{ProcessGroup, RealRun};
use charkit_adapters::{fs, posix};
...
fn a_zero_or_unparseable_pgid_is_dropped_rather_than_signalled() {
    ...
    assert!(owned_rows(machine.home.path()).is_empty());
    assert!(
        fs::stat(&repo) == charkit_core::reap::PathStat::Present,
