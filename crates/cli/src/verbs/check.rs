...
//! appears here it belongs in [`charkit_core::schedule`] instead, where a unit
//! test can reach it.
//!
//! ```text
...
use charkit_adapters::{fs, git, runs};
use charkit_core::config::{ResolvedCheck, ResolvedComponent, ResolvedConfig, Scope};
use charkit_core::ctx::{Clock, Fetch, Run, RunRequest, SpawnErrorKind, StdioMode};
use charkit_core::dispatch::{Dispatch, Journal, Scrub};
use charkit_core::envelope::{CheckData, CheckDryRun, Envelope};
use charkit_core::error::{CharError, ConfigWhere, ErrClass, Status};
use charkit_core::lease::{self, LeaseId, LeaseKind, Policy};
use charkit_core::reap::PathStat;
use charkit_core::run::{RunId, RunRecord};
use charkit_core::schedule::{
    self, Action, CheckId, CheckResult, EnvDelta, Event, Phase, Plan, State,
};
use charkit_core::select::{self, Selection, Selector};
...
fn assigned_ports<R: Run, C: Clock, F: Fetch>(
    ...
    match (block, declares_ports) {
        (Some(block), _) => {
            charkit_core::ports::assign_ports(config, block, &workspace.config_label)
        }
    ...
    Ok(Plan {
        ...
        needs: check
            ...
            .filter_map(|need| match need {
                charkit_core::config::Need::Check(check) => Some(CheckId::new(check)),
                charkit_core::config::Need::Component(_) => None,
            })
            .collect(),
        log: None,
        skip,
...
fn dry<R: Run, C: Clock, F: Fetch>(
    ...
    preview.would_reap = charkit_core::run::runs_to_reap(&present, app.machine.run_retention, &[])
        .iter()
        .map(RunId::to_string)
        .collect();
...
fn acquire<R: Run, C: Clock, F: Fetch>(
    ...
    let mut denials: Vec<charkit_core::lease::WaitingOn> = Vec::new();
    ...
    if let Some(first) = denials.first() {
        let holder = match first {
            charkit_core::lease::WaitingOn::Exclusive { held_by, .. } => Some(held_by.clone()),
            charkit_core::lease::WaitingOn::CpuSlot { .. }
            | charkit_core::lease::WaitingOn::Run { .. } => None,
        };
...
fn collect_children(workspace: &Workspace, it: &mut Loop, queue: &mut Vec<Event>) {
    ...
    // right to refuse.
    let finished: Vec<(CheckId, charkit_core::ctx::RunOutput)> = it
        .children
        .iter_mut()
...
fn entropy<R: Run, C: Clock, F: Fetch>(app: &App<R, C, F>) -> u64 {
    let pid = charkit_adapters::posix::pid() as u64;
