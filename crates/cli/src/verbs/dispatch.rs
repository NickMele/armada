...
use charkit_core::envelope::{DispatchData, Envelope};
...
pub fn run<R: Run, C: Clock, F: Fetch>(
    ...
    let ports = match block {
        Some(block) => charkit_core::ports::assign_ports(&config, block, &workspace.config_label)?,
    ...
    let stdio = if json || entry.stdio == charkit_core::config::Stdio::Pipe {
        StdioMode::Capture
    } else {
        StdioMode::Inherit
    };
