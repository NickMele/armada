...
use charkit_core::envelope::{Envelope, PortReport, ResultRow, StatusData, Unreclaimed};
use charkit_core::error::{CharError, Status};
use charkit_core::lease::is_cold;
use charkit_core::ports::PortState;
    ...
    for row in &selected {
        ...
        if exists {
            if let Some(config) = load_foreign_config(&row.path, &app.machine) {
                if let Ok(ports) = charkit_core::ports::assign_ports(&config, row.ports, "char.yml")
                {
                    result.ports = ports
                        .into_iter()
                        .map(|(name, port)| {
