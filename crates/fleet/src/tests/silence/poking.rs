//! What the vigil does once it has found a silence: a nudge, a second nudge,
//! and then an escalation.
//!
...
//! would fail.
//!
//! The last case sends nothing and reads the poke's own words instead, which is
//! the prompt contract's claim about the nudge rather than the vigil's.
...
use crate::silence::{Liveness, Poke, Vigil};
use crate::tests::planted::Held;
use crate::tests::silence::{
    a_drone_that_goes_quiet, after_the_threshold, called, one_step, started, watching, Fixture,
    QUIET_AFTER,
};
...
/// A Fleet watching one step with that Drone on it.
fn a_watched_fleet(
    home: &TempDir,
    harness: FakeHarness,
    clock: Arc<Held>,
