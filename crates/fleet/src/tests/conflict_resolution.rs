...
///
/// **A turn is not a clock** (`#1436`). This read the record only after
/// turning, and turned again whenever it had not arrived — so on a busy
/// machine, where the spawn takes longer to land, the case took turns it does
/// not take on an idle one. The fixture's Drone speaks once and exits, and the
/// first turn taken after it has gone stops its step as `RunEnded` and
/// escalates the Job, which is what the merge line saw: `no Drone reached
/// summarise: Some(Escalated)`. The wait was deciding the case it was waiting
/// on, and how many turns it took was a reading of the machine.
///
