...
//!
//! **Every other crate takes its instant as an argument.** `core-model`, `store`, `config` and the
//! gate all say it: a function reading its own inputs from the process cannot be tested or replayed.
//! [`Clock`] is the one trait, one method, held by Fleet and passed down as a [`Timestamp`] — a test
//! plants a clock answering a fixed string and the system below Fleet becomes deterministic.
//!
//! **The format is `Timestamp`'s, spelled once** — RFC3339, UTC, millisecond precision,
//! `core_model::Timestamp`'s own contract. Computed here rather than taken from a date library
//! because the computation is the civil-date arithmetic below, and a dependency for it would be
//! a dependency in the crate that spawns Drones.
//!
//! **No parsing counterpart, and there will not be one here** — a stored instant is compared,
//! ordered and displayed as text; the moment something wants arithmetic on one, the type that
//! holds it changes rather than this module growing a reader.
