...
use core_model::{Attempt, JobId, ResolvedStep, StepFrame, StepId};
...
pub async fn show(
    ...
    let mut serving = match Served::spawn(harness.serve(), worktree) {
        Ok(serving) => serving,
        Err(why) => return Shown::NotShown(NotShown::NotServed(why)),
    };
    if let Some(why) = ready(harness, worktree, coming_up, &mut serving).await {
    ...
    let ran = checks_runner::run(&harness.running(spec), worktree, budget).await;
    serving.end().await;
    match ran.exit {
        ...
        // nothing sets.
        Exit::Code(0) => collected(harness, worktree),
...
fn collected(harness: &Harness, worktree: &Path) -> Shown {
...
pub fn kept(
    ...
    let Some(relative) = run_dir(handle, step, attempt) else {
        return Vec::new();
    };
...
fn run_dir(handle: &str, step: &StepId, attempt: Attempt) -> Option<String> {
    let id = step.as_str();
    if !crate::check_output::one_component(handle) || !crate::check_output::one_component(id) {
        return None;
    }
    Some(format!(".armada/frames/{handle}/{id}.{attempt}"))
}
    ...
    W::Error: std::error::Error + Send + Sync + 'static,
{
    ...
    pub(crate) async fn showed(
        ...
        let shown = show(harness, spec, worktree, ComingUp::of(budget), budget).await;
        let frames = match shown {
            Shown::NotShown(why) => return Ok(Some(why)),
            Shown::Nothing => return Ok(None),
        ...
        self.store()
            .lock()
            .await
            .record_step_frames(job_id, step, &kept, &self.now())
