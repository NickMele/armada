...
/// The content a path holds at the branch tip, ignoring the working
/// directory entirely.
pub(crate) fn content_at_tip(
    worktree: &Worktree,
    path: &str,
) -> Result<Option<String>, CommitWorkError> {
    let repo = open(worktree)?;
    let Some(parent) = tip(&repo) else {
        return Ok(None);
    };
    let unreadable = |cause| CommitWorkError::NotStaged {
        worktree: worktree.path().to_string(),
        cause,
    };
    let tree = parent.tree().map_err(unreadable)?;
    let Ok(entry) = tree.get_path(Path::new(path)) else {
        return Ok(None);
    };
    let blob = repo.find_blob(entry.id()).map_err(unreadable)?;
    std::str::from_utf8(blob.content())
        .map(str::to_string)
        .map(Some)
        .map_err(|_| CommitWorkError::NotUtf8 {
            worktree: worktree.path().to_string(),
            path: path.to_string(),
        })
}

/// Commit this exact content at `path`, on top of the branch tip, and touch
/// the working directory nowhere in the process.
///
/// **The index is brought to what was committed, never to what the working
/// directory holds.** Left alone, the index would still read the tip's old
/// blob for `path` after the branch moved, so `git status` would show it
/// twice over — once staged, against the commit that just superseded it, and
/// once more against whatever the working directory holds. Writing the
/// *given* content straight into the index — [`Index::add_frombuffer`], which
/// never reads the working directory — is what leaves a person's `git status`
/// showing one thing: the working directory against what is now committed.
pub(crate) fn commit_content(
    ...
    let file_mode = base
        ...
        .map(|entry| mode(entry.filemode() as u32))
        .unwrap_or(FileMode::Blob);
    let blob_id = repo.blob(content.as_bytes()).map_err(staged)?;
    let mut update = TreeUpdateBuilder::new();
    update.upsert(path, blob_id, file_mode);
    let tree_id = update.create_updated(&repo, &base).map_err(staged)?;
    if tree_id == base.id() {
        return Ok(Committed::NothingToCommit);
    }
    let committed = made(&repo, worktree, tree_id, parent.as_ref(), message, at)?;
    reflect_in_the_index(&repo, path, content.as_bytes(), file_mode).map_err(staged)?;
    Ok(committed)
}

/// Write `content` into the index at `path`, from the buffer rather than the
/// working directory — [`Index::add_frombuffer`] never reads a file.
fn reflect_in_the_index(
    repo: &Repository,
    path: &str,
    content: &[u8],
    file_mode: FileMode,
