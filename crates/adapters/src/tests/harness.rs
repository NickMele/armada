...
fn armadas_own_tools_are_in_a_toolbelt_that_was_granted_nothing() {
    ...
    assert_eq!(
        ...
        "a Drone granted nothing else still reports, still declares its scope, \
         ...
         Fleet holds, can still leave another Job's Drone a note and can still \
         draft the fix for a test broken on main, because none of the eight is \
         one of the grants — and a Drone denied one is denied silently"
    );
}
