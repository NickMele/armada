...
/// **The Drone's own record of what it said was broken on main.** Fleet keeps
/// the claim, but only this row says which Drone said it and when. #999.
#[test]
fn a_fix_row_names_the_test_said_to_be_broken_on_main() {
    let read = read(
        r#"{"type":"assistant","message":{"content":[
             {"type":"tool_use","id":"a","name":"mcp__armada__draft_fix",
              "input":{"check":"test","test":"store::reads_the_last_row",
                       "failure":"exited 101","title":"Fix the last-row read",
                       "workflow":"bug","brief":"It fails on main.",
                       "acceptance_criteria":[]}}]}}"#,
    );
    let [DroneEvent::Called { detail, .. }] = &read[..] else {
        panic!("{read:?}");
    ...
    assert_eq!(
        detail.shown(),
        "store::reads_the_last_row under test, broken on main"
    );
    assert!(ipc::mcp::FIX_FIELDS.contains(&"check") && ipc::mcp::FIX_FIELDS.contains(&"test"));
}
...
/// **A Helm session's own tools arrive prefixed by the MCP server**, never
/// bare — `mcp__armada-fleet__ask_person_to_approve`, `ipc::door::SERVER`
/// named. `detail` reads `input` alone, so the prefix on `name` changes
/// nothing about what this decodes; `packages/screens/src/helm-thread.ts` is
/// where the prefix has to be read, and this is the row it reads.
#[test]
fn an_approval_ask_names_the_job_it_is_for_prefixed_by_helms_own_server_too() {
    let read = read(
        r#"{"type":"assistant","message":{"content":[
             {"type":"tool_use","id":"a","name":"mcp__armada-fleet__ask_person_to_approve",
              "input":{"job_id":"01JOB9"}}]}}"#,
    );
    let [DroneEvent::Called { tool, detail, .. }] = &read[..] else {
        panic!("{read:?}");
    };
    assert_eq!(tool, "mcp__armada-fleet__ask_person_to_approve");
    assert_eq!(detail.shown(), "01JOB9");
}
