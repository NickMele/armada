...
fn on_screen(
    ...
    match action {
        ...
        // the parser uses for every other claimed name — and it says so on the
        // screen rather than by ending it.
        Action::Chat => Done::said(
            "`armada helm` is not built yet — M3; enter boards the selected Job".to_string(),
        ),
    }
}
