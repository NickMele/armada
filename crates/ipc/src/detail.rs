...
pub use confidence::{
    AreaRow, ChangedTestRow, FindingRow, JobConfidence, OpenedBecause, ProvesRow, Says,
    TestsSection, UntestedRow,
};
...
pub struct JobDetail {
    ...
    /// absent from a Fleet older than that -- which a reader draws as no
    /// setting at all rather than as the default. Filled after
    ...
    /// **Since 13.24**, set after [`JobDetail::of`] like `model_override`. #903.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct JobSpend {
    ...
    /// signalled mid-run leaves none — one Job had two such Drones over 277
    /// and 299 seconds and read as $5.28 against a $5 cap.
    ///
